...
public class SlidingRecyclerAdapter extends RecyclerView.Adapter<SlidingRecyclerAdapter.MyViewHolder> implements SlidingButtonView.IonSlidingButtonListener {

    private static String TAG="SlidingRecyclerAdapter";
    private Context mContext;
    private IonSlidingViewClickListener mIDeleteBtnClickListener;
    private List<ChoresListMode> mDatas = new ArrayList<ChoresListMode>();
    private SlidingButtonView mMenu = null;
    List<ChoresL28TDB> mChoresL28TDBs;
    public SlidingRecyclerAdapter(Context context, IonSlidingViewClickListener ionSlidingViewClickListener , List<ChoresListMode> data, List<ChoresL28TDB> choresL28TDBs) {
        mContext = context;
        mIDeleteBtnClickListener = ionSlidingViewClickListener;
        mDatas=data;
        mChoresL28TDBs=choresL28TDBs;
    }
    public void updateData( List<ChoresListMode> mDatas){
        this.mDatas = mDatas;
    ...
    public void onBindViewHolder(final MyViewHolder holder, int position) {
        ...
        ChoresListMode choresListMode= mDatas.get(position);
        holder.textView.setText(choresListMode.getName());        //设置内容布局的宽为屏幕宽度
        ...
        if(mChoresL28TDBs==null||mChoresL28TDBs.size()<=0){
            holder.c_r_item_date_ll.setVisibility(View.GONE);
        }else{
            LogUtils.i(TAG,"mChoresL28TDBs.size"+mChoresL28TDBs.size());
            for(ChoresL28TDB choresL28TDB:mChoresL28TDBs){
                if(choresL28TDB.getChoresId()==choresListMode.getChoresId()){
                    ...
                    holder.c_r_item_gold_tv.setText(" x"+choresL28TDB.getGold()+"");
                    holder.c_r_item_date_tv.setText(getDateString(choresL28TDB.getDate()));
...
//        mDatas.remove(position);
    ...
    class MyViewHolder extends RecyclerView.ViewHolder {
        ...
        public MyViewHolder(View itemView) {
            ...
            ((SlidingButtonView) itemView).setSlidingButtonListener(SlidingRecyclerAdapter.this);        }    }
}
